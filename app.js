...
} from "./controllers/HostelDashboardController.js";
...
app.use(
	cors({
		origin: [
			"http://localhost:5173",
			"https://s9x3g1x0-5173.inc1.devtunnels.ms",
		],
		credentials: true,
...
io.use((socket, next) => {
	try {
		...
		if (!token ) {
			console.log("User Authentication has been failed  !! ");
			return next(new Error("Authentication failed"));
		}

		console.log("The role token is : ", role_token);
		...
		console.log("The role of the user is : ", decodedRole.role);
...
io.on("connection", async (socket) => {
	...
	try {
		...
		// Reduced update interval to 1000ms for near real-time updates
		...
		socket.on("getWardenDashboardData", async () => {
			...
			const data = await updateWardenDashboardData(socket.role); // Pass user object
